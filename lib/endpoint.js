module.exports = function (inherits, EventEmitter) {
	...
	function Endpoint(http, ip, port, options) {
		...
		this.timeoutCheck =
			function () {
				...
				if (self.requestRate < 5 && options.ping) {
					self.ping(options.ping)
				}
			}
		this.timeoutInterval = setInterval(this.timeoutCheck, this.resolution)
	}
	...
	Endpoint.prototype.ping = function (path) {
		this.request({ path: path, method: 'GET' }, gotPingResponse)
	}
