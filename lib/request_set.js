...
function handleResponse(err, response, body, respondingNode) {
	...
	if (err) {
        if (!reqInfo.failedNodes) {
            reqInfo.failedNodes = [];
        }
        reqInfo.failedNodes.push(respondingNode);
	...
	if (this.callback) {
        reqInfo.numRetries = this.attempts - this.attemptsLeft;
        if (fromNode) {
            reqInfo.respondingNode = respondingNode.name;
        }

		this.callback(err, response, body, reqInfo)
		this.callback = null
	}
}
