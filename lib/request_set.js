...
//  retryDelay: minimum ms to wait before first retry using exponetial backoff (20)
// }
...
function exponetialBackoff(attempt, delay) {
...
function handleResponse(err, response, body) {
	this.attemptsLeft--
	if (err) {
		var delay = (err.delay === true) ? exponetialBackoff(this.attempts - this.attemptsLeft, this.delay) : err.delay
