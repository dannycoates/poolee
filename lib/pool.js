...
module.exports = function(inherits, EventEmitter, Endpoint, RequestSet) {
  ...
  Pool.request = function(options, data, cb) {
    ...
    var host = uri.host
    var port = uri.port || secure ? 443 : 80
  ...
  Pool.prototype.rate = function() {
    return this.nodes.reduce(function(a, b) {
      return a + b.requestRate
    }, 0)
  }

  Pool.prototype.requestCount = function() {
    return this.nodes.reduce(function(a, b) {
      return a + b.requestCount
    }, 0)
  }

  Pool.prototype.addNode = function(name) {
    var ip_port = name.split(':')
    var ip = ip_port[0]
    var port = +ip_port[1]
    if (port > 0 && port < 65536) {
      var node = new Endpoint(this.http, ip, port, this.options)
  ...
  Pool.prototype.removeNode = function(name) {
